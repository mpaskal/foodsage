...
const FoodItemModal = ({
  ...
  useEffect(() => {
    ...
    updateExpirationDate(form.category, form.storage, form.purchasedDate);
  }, [form.category, form.storage, form.purchasedDate]);
  ...
  const updateExpirationDate = (category, storage, purchasedDate) => {
    const expirationDate = calculateExpirationDate(
      category,
      storage,
      purchasedDate
    );
    if (expirationDate && expirationDate.toISOString) {
      console.log(
        `Updating expiration date to: ${expirationDate
          .toISOString()
          .substring(0, 10)}`
      );
      handleChange({
      ...
      console.error("Calculated expiration date is invalid:", expirationDate);
    }
  };
